...
\centerImage{figures/jvmRendererComponent.pdf}{UML Component - JVM Renderer Component}{jvm-renderer}{0.5}
...
\centerImage{figures/proof.pdf}{UML Component Diagram - First Proof of Concept Architecture}{proof}{1}
...
\begin{itemize}
	\item \textbf{DoppioJVM}: an open-source project that aims to provide a fully-featured Java Virtual Machine (JVM) written entirely in JavaScript\footnote{http://doppiojvm.org};
	\item \textbf{JSweet}: a Java to JavaScript transpiler that converts Java source code to JavaScript source code\footnote{https://www.jsweet.org/};
	\item \textbf{TeaVM}: a Java to JavaScript transpiler that translates Java bytecode to JavaScript\footnote{https://teavm.org/};
	\item \textbf{J2CL}: a Google-made transpiler that converts Java source code to JavaScript, using the Closure Library\footnote{https://github.com/google/j2cl}.
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{Poor Library support}: Basic support for common Java APIs can be found in the J4TS\footnote{https://github.com/j4ts/j4ts} project. It is expected that more APIs will be supported in the future, however, it is impossible to cover all the libraries out there;
...
\paragraph{Weaknesses and Limitations} J2CL has some known and documented limitations\footnote{https://github.com/google/j2cl/blob/master/docs/limitations.md}, including:
\begin{itemize}
...
\centerImage{figures/kotlin-multiplatform.png}{Kotlin Multiplatform - Platforms}{kotlin-mp}{0.5}
...
Kotlin Multiplatform\footnote{https://kotlinlang.org/docs/multiplatform.html} may have many applications, but the following are some of the most prevalent ones:
...
Kotlin Multiplatform enables code sharing through its mechanisms. The source code is organized into source sets, each targeting a specific platform. \fullref{fig:kotlin-mp-sourcesets}\footnote{https://kotlinlang.org/docs/multiplatform--on-platforms.html} shows the dependencies among source sets. As shown, if there is business logic that applies to all platforms, it can be shared in the common source set, eliminating the need to write the same code multiple times.

\centerImage{figures/sourcesets.png}{Kotlin Multiplatform - Source set dependencies}{kotlin-mp-sourcesets}{1}

\subsection{Handling Third-Party Dependencies}
\label{ssec:handling-third-party-dependencies}
All softwares requires a collection of libraries to work smoothly. A Kotlin Multiplatform project could depend on multiplatform libraries that are compatible with all target platforms, platform-specific libraries, and even other multiplatform projects~\footnote{https://kotlinlang.org/docs/multiplatform-add-dependencies.html}.\newline
